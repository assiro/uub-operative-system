 ...
 static u8 ADC_LVDS[3] = {0x00,0x14,0xA0} ; // ADC bus configuration LVDS interleave
 static u8 ADC_RESET[3] = {0x00,0x00,0x3C} ; // ADC reset
...
// k= 0x02;
     for (k = SPI_SELECT_ADC0; k < (SPI_SELECT_ADC4 + 0x01); k++) //ciclo per tutti gli ADC impostati uguali
      {
        XSpiPs_SetSlaveSelect(&Spi, k);                         // Assert the chip select
        if (Status != XST_SUCCESS) { return XST_FAILURE; }
        XSpiPs_PolledTransfer(&Spi, ADC_RESET, ReadBuffer, 3);// RESET
        if (Status != XST_SUCCESS) { return XST_FAILURE; }

        XSpiPs_PolledTransfer(&Spi, ADC_LVDS, ReadBuffer, 3);// send bus configuration
        if (Status != XST_SUCCESS) { return XST_FAILURE; }
/**/
        XSpiPs_PolledTransfer(&Spi, ADC_VREF, ReadBuffer, 3);// send VREF configuration
        if (Status != XST_SUCCESS) { return XST_FAILURE; }

        XSpiPs_PolledTransfer(&Spi, TestModeOff, ReadBuffer, 3);// send normal configuration
//        XSpiPs_PolledTransfer(&Spi, TestModeMS, ReadBuffer, 3);// send middle
//        XSpiPs_PolledTransfer(&Spi, TestModeRM, ReadBuffer, 3);// send RAMPA MODE
//        XSpiPs_PolledTransfer(&Spi, TestModeA5, ReadBuffer, 3);// send A5 MODE
        if (Status != XST_SUCCESS) { return XST_FAILURE; }

//        XSpiPs_PolledTransfer(&Spi, ADC_DELAY, ReadBuffer, 3);// send delay
//        if (Status != XST_SUCCESS) { return XST_FAILURE; }
